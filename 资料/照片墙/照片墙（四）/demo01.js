 ...
 * 目标：检测与拖拽图片碰撞的其它图片中，哪一个图片与之距离最短，给这个距离最短的图片加上col样式
 *       且去掉其它图片上的col样式（在第三步的基础上完成）
 *
 *  求两点之间的距离思路：利用三角形“勾股定理”  斜边平方 = 两直角边平方的和  c平方 = a平方 + b平方
 *                        然后对c进行开平方，这里用Math对象上面的方法，自己可去查询
 *
 *  最小距离思路：回忆起之前讲过的 怎么求数组里最小值的那个方法
 */
$(function () {
    //获取所有的li
    var $lis = $("#list li");
